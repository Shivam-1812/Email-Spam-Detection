...
<pre> ``` Spam-Detector/ │ ├── data/ │ ├── SMSSpamCollection │ └── readme │ ├── spam_train.py # trains and saves the model ├── test_model.py # predicts from a file ├── analyze_errors.py # evaluates and finds misclassified samples ├── demo_threshold.py # console demo with custom threshold ├── app.py # Flask web app ├── spam_model.joblib # trained model ├── test_messages.txt ├── test_results.csv └── misclassified.csv ``` </pre>
